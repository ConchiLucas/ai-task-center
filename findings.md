...
# 2026-07-19 全量 TTS MinIO 一致性审计

- `public.word_clean_tts` 22,098 条全部成功、全部引用实际存在的 MinIO 对象，大小差异为 0。
- `public.word_clean_best_sentence` 22,098 条中 22,088 条成功；精确 10 条没有有效存储引用，确定性 MinIO key 和对应本地 WAV 均不存在。
- 这 10 条对应任务配置 1 结果为 7 条错误 `SUCCESS`、3 条 `FAILED`；来源状态为 9 条 `pending`、1 条残留 `running`。
- 任务配置 4 的 22,098 条正式成功结果中，21,888 条业务事实已经迁移 MinIO，但结果快照仍是 Worker 本地 URL；其余 210 条已经是 MinIO 快照。
- `ai-file-navigation/word_clean_tts/` 有 44,462 个对象；当前两张业务表引用 44,186 个，剩余 276 个本次只读归属，不删除。
- 精确修复后新增 10 个最佳句子对象，MinIO 前缀对象总数为 44,472；当前业务引用增加为 44,196，未引用对象仍为 276。
- 6 条历史结果的句子快照与当前来源存在拼写或重音符号差异；来源 guard 正确拒绝回填旧音频，刷新当前来源并重新生成后通过。
- 276 个未引用对象全部能映射到 task_config 1 历史任务结果，没有当前来源未引用或完全无法映射对象。
